...
data_cfg = {
    ...
    # },
    'train': {
        'root': '/group2/ct/rex/data/video',
        'subset_name': '/group2/ct/rex/data/cache3/v1-16-memmap/vgg-train.tsv',
        'normalize_audio': True,
    },
    'test': {
        'root': '/group2/ct/rex/data/video',
        'subset_name': '/group2/ct/rex/data/cache3/v1-16-memmap/vgg-test.tsv',
        'normalize_audio': False,
    },
    'val': {
        'root': '/group2/ct/rex/data/video',
        'subset_name': '/group2/ct/rex/data/cache3/v1-16-memmap/vgg-val.tsv',
        'normalize_audio': False,
    },
}
...
def extract():
    ...
    for split in data_cfg.keys():
        ...
        for curr_iter, data in enumerate(tqdm(loader)):
            ...
            # Condition
            extra_condition = feature_extractor.get_extra_condition(audio)
            output['extra_condition'] = extra_condition.detach().cpu()
        ...
        if local_rank == 0:
            ...
            for t in tqdm(sorted(os.listdir(this_latent_dir))):
                ...
                for bi in range(bs):
                    ...
                    output_data['extra_condition'].append(data['extra_condition'][bi])
