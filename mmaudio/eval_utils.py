...
large_44k_v2 = ModelConfig(model_name='large_44k_v2',
                           ...
                           bigvgan_16k_path=None,
                           mode='44k')
medium_w_loudness_16k = ModelConfig(model_name='medium_w_loudness_16k',
                        model_path=Path('./weights/mmaudio_medium_w_loudness_16k.pth'),
                        vae_path=Path('./ext_weights/v1-16.pth'),
                        bigvgan_16k_path=Path('./ext_weights/best_netG.pt'),
                        mode='16k')
all_model_cfg: dict[str, ModelConfig] = {
    ...
    'medium_w_loudness_16k': medium_w_loudness_16k,
}
...
def generate(
    ...
    *,
    ...
    extra_cond: Optional[Union[torch.Tensor, list]] = None,
    input_audio: Optional[torch.Tensor] = None,
    start_time: Optional[float] = None,
    mask_level: Optional[int] = None,
    cfg_strength_extra: Optional[float] = None,
    med_filter_size: Optional[int] = None,
    return_extra: Optional[bool] = False,
) -> torch.Tensor:
    device = feature_utils.device
    dtype = feature_utils.dtype
    if cfg_strength_extra is None:
        cfg_strength_extra = cfg_strength
    ...
    if extra_cond is not None:
        if type(extra_cond) is list:
            aug_arg["temporal_mask"] = "condition"
        elif type(extra_cond) is torch.Tensor:
            extra_cond = extra_cond.to(device, dtype, non_blocking=True)
            extra_cond = feature_utils.get_extra_condition(extra_cond)
        else:
            raise ValueError(f'Unknown type for extra_cond: {type(extra_cond)}')

        if mask_level is not None:
            aug_arg["mask_level"] = mask_level

        if med_filter_size is not None:
            aug_arg["med_filter_size"] = med_filter_size
    else:
        extra_cond = None

    preprocessed_conditions = net.preprocess_conditions(clip_features, sync_features, text_features, extra_cond, aug_arg)
    ...
    proxy_conditions = net.preprocess_conditions(clip_features, sync_features, text_features, extra_cond, aug_arg)
    empty_conditions.extra_cond = proxy_conditions.extra_cond

    cfg_ode_wrapper = lambda t, x: net.ode_wrapper_dual(t, x, preprocessed_conditions, proxy_conditions, empty_conditions, cfg_strength, cfg_strength_extra)
    ...
    if input_audio is None:
        ...
        if input_audio.ndim == 1:
            input_audio = input_audio.unsqueeze(0).tile(bs, 1)
        dist = feature_utils.encode_audio(input_audio.to(device, dtype, non_blocking=True))
        a_randn = torch.empty_like(dist.mean).normal_(generator=rng)
        x1_input = dist.mean + dist.std * a_randn
        x1_input = net.normalize(x1_input.transpose(1, 2))
        x1 = fm.to_data_given_conditional_data(cfg_ode_wrapper, x1_input, start_time, rng)
