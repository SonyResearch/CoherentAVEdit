...
def read_audio_frames(video_path: Path, sampling_rate: int, start_sec: float, end_sec: float) -> np.ndarray:
    all_frames = []

    with av.open(video_path) as container:
        try:
            expected_length = int(sampling_rate * (end_sec - start_sec))
            output_frames = np.zeros(expected_length)
            audio_stream = next(s for s in container.streams if s.type == 'audio')
            container.seek(int(start_sec / audio_stream.time_base))
            ...
            for packet in container.demux(audio_stream):
                for frame in packet.decode():
                    ...
                    if frame.time >= start_sec:
                        ...
                        if frame_np.ndim > 1: # stereo -> mono
                            frame_np = np.mean(frame_np, axis=0)
                        all_frames.append(frame_np)
            tmp = np.concatenate(all_frames, axis=0)
            if original_sample_rate != sampling_rate:
                tmp = librosa.resample(tmp.astype(np.float32), orig_sr=original_sample_rate, target_sr=sampling_rate)
            output_frames[:min(tmp.shape[0], expected_length)] = tmp[:min(tmp.shape[0], expected_length)]
