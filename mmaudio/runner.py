...
class Runner:
    ...
    def __init__(self,
        ...
        self.network = DDP(get_my_mmaudio(cfg.model,
                                          ...
                                          extra_condition_info=cfg.extra_condition_info if "extra_condition_info" in cfg else None).cuda(),
    ...
    def train_fn(
        ...
        # apply data augmentation as well as cfg training for extra_cond
        aug_arg = None
        if extra_cond is not None:
            aug_arg = {}
            # median filter
            aug_arg["med_filter_size"] = "random"

            # temporal masking
            aug_arg["temporal_mask"] = "random"

            # mask level
            if self.scheduler.last_epoch < self.num_iterations // 2:
                # do not feed extra conditions for the first half of training
                aug_arg["mask_level"] = 0
            else:
                aug_arg["mask_level"] = "random"

        pred_v = self.network(xt, clip_f, sync_f, text_f, t, extra_cond, aug_arg)
    ...
    def val_fn(
        ...
        pred_v = self.network(xt, clip_f, sync_f, text_f, t, extra_cond)
    ...
    def train_pass(self, data, it: int = 0):
        ...
        with torch.amp.autocast('cuda', enabled=self.use_amp, dtype=torch.bfloat16):
            ...
            extra_cond = data['extra_condition'].cuda(non_blocking=True) if 'extra_condition' in data else None
            ...
            if it % self.log_extra_interval == 0:
                ...
                if extra_cond is not None:
                    unmasked_extra_cond = extra_cond.clone()
            x1, loss, mean_loss, t = self.train_fn(clip_f, sync_f, text_f, a_mean, a_std, extra_cond)
        ...
        with torch.amp.autocast('cuda', enabled=self.use_amp,
            ...
            try:
                if it % self.log_extra_interval == 0:
                    ...
                    extra_cond = unmasked_extra_cond[0:1] if extra_cond is not None else None
                    conditions = self.network.module.preprocess_conditions(clip_f, sync_f, text_f, extra_cond)
    ...
    def validation_pass(self, data, it: int = 0):
        ...
        with torch.amp.autocast('cuda', enabled=self.use_amp, dtype=torch.bfloat16):
            ...
            a_mean = data['a_mean'].cuda(non_blocking=True)
            a_std = data['a_std'].cuda(non_blocking=True)
            extra_cond = data['extra_condition'].cuda(non_blocking=True).clone() if 'extra_condition' in data else None
            ...
            self.log.data_timer.end()
            loss, mean_loss, t = self.val_fn(clip_f.clone(), sync_f.clone(), text_f.clone(), x1, extra_cond)
    ...
    def load_weights_in_memory(self, src_dict, strict=True):
        self.network.module.load_weights(src_dict, strict=strict)
    ...
    def load_weights(self, path, strict=True):
        ...
        self.load_weights_in_memory(src_dict, strict=strict)
