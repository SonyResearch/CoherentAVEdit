...
def main():
    ...
    feature_extractor = FeaturesUtils(tod_vae_ckpt=tod_vae_ckpt, 
    ...
    for i, batch in tqdm(enumerate(dataloader), total=len(dataloader)):
        ...
        dist = feature_extractor.encode_audio(waveforms)
        extra_condition = feature_extractor.get_extra_condition(waveforms)

        a_mean = dist.mean.detach().cpu().transpose(1, 2)
        a_std = dist.std.detach().cpu().transpose(1, 2)
        text_features = text_features.detach().cpu()
        extra_condition = extra_condition.detach().cpu()
    ...
    if local_rank == 0:
        ...
        for t in tqdm(latents):
            ...
            for bi in range(bs):
                ...
                output_data['mean'].append(data['mean'][bi].cpu())
                output_data['std'].append(data['std'][bi].cpu())
                output_data['text_features'].append(data['text_features'][bi].cpu())
                output_data['extra_condition'].append(data['extra_condition'][bi].cpu())
