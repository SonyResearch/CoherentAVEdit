...
class FeaturesUtils(nn.Module):
    ...
    def compile(self):
        ...
        self.get_extra_condition = torch.compile(self.get_extra_condition)
    ...
    @torch.inference_mode()
    def get_extra_condition(self, x, n_split_max = 16) -> torch.Tensor:
        assert hasattr(self, "mel_converter"), 'Mel converter is not loaded'
        tmp_norm_fn = self.mel_converter.norm_fn
        self.mel_converter.norm_fn = torch.log10
        spec = self.mel_converter(x)
        self.mel_converter.norm_fn = tmp_norm_fn
        B, M, T = spec.shape
        mel_freq = librosa.mel_frequencies(n_mels=M, fmin=self.mel_converter.fmin, fmax=self.mel_converter.fmax)
        weight = torch.tensor(librosa.A_weighting(mel_freq), device=spec.device)
        spec_weighted = spec + weight.view(1, M, 1)
        spec_weighted = torch.exp(spec_weighted / torch.log10(torch.tensor(math.e)))

        assert M % n_split_max == 0, 'num of mels must be a multiple of n_split_max'
        n_split = n_split_max
        loudness_all = []
        while n_split > 0:
            loudness = spec_weighted.view(B, n_split, M // n_split, T).mean(dim=2)
            loudness_all.append(torch.log10(loudness).to(dtype=x.dtype))
            n_split = n_split // 2
        loudness_all = torch.cat(loudness_all, dim=1)
