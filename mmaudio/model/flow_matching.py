...
class FlowMatching:
    ...
    def to_data_given_conditional_data(self, fn: Callable, x1: torch.Tensor, t: float, generator: torch.Generator = None) -> torch.Tensor:
        assert 0 < t < 1
        xt = t * x1 + (1 - t) * torch.randn(x1.shape, generator=generator, dtype=x1.dtype, device=x1.device)
        return self.run_t0_to_t1(fn, xt, t, 1)
